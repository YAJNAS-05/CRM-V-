...
const DealListPage: React.FC = () => {
  ...
  useEffect(() => { fetchDeals() }, [page, pageSize, stageFilter])

  const fetchDeals = async () => {
    try {
      setIsLoading(true)
      const resp = stageFilter ? await dealApi.getByStage(stageFilter, page, pageSize) : await dealApi.getAll(page, pageSize)
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <div className="flex bg-gray-900 rounded-full p-0.5">
            <Link to="/crm/deals" className="px-4 py-1.5 text-sm font-medium rounded-full bg-white text-gray-900 shadow-sm">List</Link>
            <Link to="/crm/deals/kanban" className="px-4 py-1.5 text-sm font-medium rounded-full text-gray-400 hover:text-white transition">Pipeline</Link>
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Deals</h1>
            <p className="text-sm text-gray-500 mt-0.5">{totalItems} total records</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link to="/crm/deals/new" className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition">
