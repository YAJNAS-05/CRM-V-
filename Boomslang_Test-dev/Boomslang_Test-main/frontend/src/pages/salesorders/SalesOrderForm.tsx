...
const salesOrderSchema = z.object({
  accountId: z.string().min(1, 'Account is required (create one in CRM → Accounts first)'),
  ...
  (d) => !d.orderDate || !d.expectedDelivery || d.expectedDelivery >= d.orderDate,
  { message: 'Expected delivery cannot be before order date', path: ['expectedDelivery'] }
).refine(
  (d) => {
    const pct = parseFloat(d.depositPercent || '')
    return isNaN(pct) || (pct >= 0 && pct <= 100)
  },
  { message: 'Deposit percentage must be between 0 and 100', path: ['depositPercent'] }
)
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                />
                {fieldErrors.equipmentId && <p className="mt-1 text-xs text-red-600">{fieldErrors.equipmentId}</p>}
              </div>
            </div>
          </div>
          <div>
