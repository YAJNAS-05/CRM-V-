...
import { Lead, CreateLeadRequest, Activity, CreateActivityRequest, LeadScoreCalculation } from '../../types/crm'
...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ isNew = false }) => {
  ...
  const [tab, setTab] = useState<'details' | 'notes' | 'activities' | 'score'>('details')
  ...
  const [scoreCalculation, setScoreCalculation] = useState<LeadScoreCalculation | null>(null)
  ...
  const fetchScoreCalculation = async () => {
    ...
    try {
      ...
      const resp = await leadApi.getScoreCalculation(id)
      setScoreCalculation(resp.data.data)
    } catch (err) { /* Silently ignore - score calculation is optional */ } finally { setScoreLoading(false) }
  }
    ...
    <div>
      ...
      <div className="border-b border-gray-200 mb-6">
        <div className="flex gap-6">
          {(['details', 'activities', 'notes', 'score'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} className={`pb-3 text-sm font-medium capitalize transition ${tab === t ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
              {t === 'score' ? (
                <span className="flex items-center gap-1">
                  Score
                  {scoreCalculation && (
                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                      scoreCalculation.qualificationStatus === 'HOT' ? 'bg-red-100 text-red-700' :
                      scoreCalculation.qualificationStatus === 'WARM' ? 'bg-orange-100 text-orange-700' :
                      scoreCalculation.qualificationStatus === 'COLD' ? 'bg-blue-100 text-blue-700' :
      ...
      {tab === 'score' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">Lead Score Analysis</h3>
          {scoreLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-indigo-50 rounded-lg p-4 text-center">
                  ...
                  <p className="text-3xl font-bold text-indigo-600">{scoreCalculation.totalScore}</p>
                  <p className="text-xs text-gray-400 mt-1">out of 100</p>
                </div>
                <div className="bg-blue-50 rounded-lg p-4 text-center">
                  <p className="text-xs text-gray-500 mb-1">Activity Score</p>
                  <p className="text-2xl font-bold text-blue-600">{scoreCalculation.activityScore}</p>
                  <p className="text-xs text-gray-400 mt-1">engagement points</p>
                </div>
                <div className="bg-green-50 rounded-lg p-4 text-center">
                  <p className="text-xs text-gray-500 mb-1">Predictive Score</p>
                  <p className="text-2xl font-bold text-green-600">{scoreCalculation.predictiveScore}</p>
                  <p className="text-xs text-gray-400 mt-1">demographics</p>
                </div>
              </div>
                ...
                <div className="flex-1 bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-2xl font-bold ${
                      scoreCalculation.grade === 'A' ? 'text-green-600' :
                      scoreCalculation.grade === 'B' ? 'text-blue-600' :
                      scoreCalculation.grade === 'C' ? 'text-yellow-600' :
                      scoreCalculation.grade === 'D' ? 'text-orange-600' :
                      'text-red-600'
                    }`}>
                      {scoreCalculation.grade}
                    </span>
                    <span className="text-sm text-gray-500">
                      ({scoreCalculation.grade === 'A' ? 'Excellent' :
                        scoreCalculation.grade === 'B' ? 'Good' :
                        scoreCalculation.grade === 'C' ? 'Average' :
                        scoreCalculation.grade === 'D' ? 'Below Average' :
                        'Poor'})
                    </span>
                  </div>
                </div>
                <div className="flex-1 bg-gray-50 rounded-lg p-4">
                  <p className="text-xs text-gray-500 mb-2">Qualification Status</p>
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                    ...
                    {scoreCalculation.qualificationStatus === 'HOT' && '🔥 '}
                    {scoreCalculation.qualificationStatus === 'WARM' && '☀️ '}
                    {scoreCalculation.qualificationStatus === 'COLD' && '❄️ '}
                    {scoreCalculation.qualificationStatus}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4">
                <p className="text-xs text-indigo-600 font-medium mb-1">🎯 Recommended Action</p>
                <p className="text-sm text-gray-700">{scoreCalculation.nextBestAction}</p>
              </div>
              ...
              <div className="text-xs text-gray-400">
                ...
                <ul className="list-disc list-inside mt-1 space-y-0.5">
                  <li>Job Title: Up to 20 points based on seniority (C-level, VP, Director, Manager)</li>
                  <li>Company Size: Up to 15 points based on employee count</li>
                  <li>Annual Revenue: Up to 15 points based on revenue size</li>
                  <li>Lead Source: Up to 10 points (Referral = highest)</li>
                  <li>Data Completeness: Up to 10 points for complete profiles</li>
                  <li>Activity Score: Points from email opens, calls, meetings, etc.</li>
                </ul>
              </div>
            </div>
          ) : (
            <div className="text-center py-8 text-gray-400">
              ...
              <p className="text-sm">Score calculation not available</p>
            </div>
          )}
        </div>
      )}
