...
public class FinancialCloseService {
    ...
    private void calculateFxRevaluation(LocalDate periodEnd) {
        ...
        for (Invoice invoice : invoices) {
            ...
            String refDocumentId = invoice.getId().toString();
            if (glJournalEntryRepository.existsByJournalSourceAndRefDocumentIdAndPostingDateAndIsDeletedFalse(
                    "FX_REVALUATION", refDocumentId, periodEnd)) {
                continue;
            }
            ...
            entry.setRefDocumentId(refDocumentId);
            entry.setBatchId("FXR-" + invoice.getInvoiceNumber() + "-" + periodEnd);
            ...
            entry.setPostingDate(periodEnd);
    ...
    private void createJournalEntry(String source, String debitAccount, String creditAccount,
        ...
        GlJournalEntry entry = new GlJournalEntry();
        entry.setEntity("DEFAULT");
        entry.setJournalSource(source);
        entry.setRefDocumentId(source + "-" + periodEnd);
        entry.setBatchId(source + "-" + periodEnd);
        entry.setLineNumber(1);
        entry.setDebitAccountCode(debitAccount);
        entry.setCreditAccountCode(creditAccount);
        entry.setDebitAmount(amount);
