...
import LeadConversionPanel, { type LeadConversionFormState } from '../../components/crm/LeadConversionPanel'
...
const LEAD_REQUIRED_FIELDS = ['firstName', 'lastName', 'email', 'phone', 'company'] as const
...
const LeadDetailPage: React.FC<LeadDetailPageProps> = ({ isNew = false }) => {
  ...
  const fetchLead = async () => {
    try {
      ...
      setLead(current)
      if (current) {
        const completion = deriveCompletionStatus(current, [...LEAD_REQUIRED_FIELDS])
        setIsEditing(completion === 'PENDING')
      }
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        const completion = deriveCompletionStatus(payload, [...LEAD_REQUIRED_FIELDS])
  ...
  const handleConvert = async (convertForm: LeadConversionFormState) => {
    if (!id) return
  ...
  if (isNew || isEditing) {
    const draftCompletion = deriveCompletionStatus(form, [...LEAD_REQUIRED_FIELDS])
    return (
      <div className="space-y-6">
        <CrmDetailHero
          ...
          badges={[<CompletionStatusBadge key="completion" status={draftCompletion} />]}
        />
  ...
  const recordCompletion = deriveCompletionStatus(lead, [...LEAD_REQUIRED_FIELDS])
    ...
    <div className="space-y-6">
      <CrmDetailHero
        ...
        badges={[
          ...
          lead.status,
          lead.leadSource?.replace('_', ' ') || 'Source pending',
          lead.company || 'No company',
        ]}
        actions={
          <>
            {lead.status !== 'CONVERTED' && recordCompletion === 'COMPLETED' && (
      ...
      {showConvertModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 px-4 backdrop-blur-sm">
          <LeadConversionPanel
            ...
            onConvert={async (form) => {
              try {
                await handleConvert(form)
              } catch {
