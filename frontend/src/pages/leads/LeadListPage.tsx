...
const LeadListPage: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    ...
    const converted = leads.filter((lead) => lead.isConverted || lead.status === 'CONVERTED').length

    return [
      { label: 'Records', value: totalItems },
      { label: 'New', value: newLeads },
      { label: 'Qualified', value: qualified, accentClassName: 'text-emerald-700' },
      { label: 'Converted', value: converted, accentClassName: converted > 0 ? 'text-violet-700' : 'text-slate-900' },
    ]
  }, [leads, totalItems])
  ...
  return (
    <div className="space-y-5">
      <CrmModuleHeader
        ...
        subtitle="Qualify inbound demand, manage lead hygiene, and drive conversion actions from a more structured operational view."
        metrics={metrics}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="pm-input pl-10 text-sm"
            />
          </div>
          <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(0) }} className="pm-select text-sm">
            <option value="">All Status</option>
            {STATUSES.filter(Boolean).map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)} className="pm-select text-sm">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr key={lead.id} className="cursor-pointer transition-colors hover:bg-slate-50/80" onClick={() => navigate(`/crm/leads/${lead.id}`)}>
