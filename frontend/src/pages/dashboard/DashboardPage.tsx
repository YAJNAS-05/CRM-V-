...
import { dashboardApi, ComposedDashboard } from '../../api/dashboardApi'
import {
  ReportActivity,
  ReportConversion,
  ReportDashboardKPIs,
...
const DashboardPage: React.FC<DashboardPageProps> = ({ mode = 'AUTO' }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const includeComposed = mode === 'AUTO' && userRoles.length > 0
      const results = includeComposed
        ? await Promise.allSettled([...calls, dashboardApi.getComposedDashboard(userRoles)])
        : await Promise.allSettled(calls)
      const [d, p, c, a, composed] = results
      ...
      if (composed && composed.status === 'fulfilled' && composed.value.data.data) {
        setComposedDashboard(composed.value.data.data)
      }
  ...
  const scopeLabel = kpi?.visibilityScope ?? composedDashboard?.scope ?? (mode === 'TEAM' ? 'TEAM' : mode === 'SELF' ? 'SELF' : (isManagerRole ? 'TEAM' : 'SELF'))
  ...
  return (
    <div className="space-y-6">
      ...
      {mode === 'AUTO' && composedDashboard && composedDashboard.widgets.length > 0 && (
        <div className="shell-card p-5 sm:p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[11px] uppercase tracking-[0.14em] text-slate-500 font-semibold">Role-based composition</p>
              <h2 className="text-base font-semibold text-slate-900 mt-1">Unified role widgets</h2>
            </div>
            <ScopePill scope={composedDashboard.scope} />
          </div>
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {composedDashboard.widgets.slice(0, 4).map((widget, index) => (
              <InsightTile
                key={widget.key}
                label={widget.title}
                value={typeof widget.value === 'number' ? formatCompact(widget.value) : String(widget.value)}
                tone={index % 2 === 0 ? 'blue' : 'indigo'}
                hint="Composed from your role permissions"
              />
            ))}
          </div>
        </div>
