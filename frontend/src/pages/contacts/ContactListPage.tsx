...
const ContactListPage: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    ...
    const withPhone = contacts.filter((contact) => Boolean(contact.phone || contact.mobile)).length
    const optOut = contacts.filter((contact) => contact.doNotCall || contact.emailOptOut).length

    return [
      { label: 'Records', value: totalItems },
      { label: 'Email Coverage', value: `${withEmail}/${contacts.length || 0}` },
      { label: 'Phone Coverage', value: `${withPhone}/${contacts.length || 0}` },
      { label: 'Opt Outs', value: optOut, accentClassName: optOut > 0 ? 'text-amber-700' : 'text-slate-900' },
    ]
  }, [contacts, totalItems])
  ...
  return (
    <div className="space-y-5">
      <CrmModuleHeader
        ...
        subtitle="Manage stakeholder records, communication coverage, and contact hygiene with a cleaner operational view."
        metrics={metrics}
      ...
      <div className="crm-toolbar p-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by name, email, phone..." className="pm-input pl-10 text-sm" />
          </div>
      ...
      {selectedRows.size > 0 && (
        <div className="crm-toolbar p-3 flex items-center justify-between">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr key={contact.id} className="cursor-pointer transition-colors hover:bg-slate-50/80" onClick={() => navigate(`/crm/contacts/${contact.id}`)}>
